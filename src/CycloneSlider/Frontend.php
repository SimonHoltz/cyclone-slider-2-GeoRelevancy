...
class CycloneSlider_Frontend {
    ...
    public function cycloneslider_shortcode( $shortcode_settings ) {
        ...
        //added for Nationality selector
        
        //usort($slides);
        //mySortFunction($slides)        
                
        $GB = "109.159.41.19";
        $US = "173.252.110.27";
         
        
        $matches = UseNationalitySortFunction($slides, $slider_html_id);
       foreach ($slides as $i => $value) {
            $slides[$i]['matches_nations'] = false;
            if(in_array($i,$matches )){
                 $slides[$i]['matches_nations'] = true;
            }
        }
        //var_dump($matches);
         $extraOut = "";
        //$extraOut .= " ". $slides. " " . ip_info($GB, "countrycode")." <br> ";
       // var_dump($slides);
        //$extraOut .= " <div style='overflow:auto;'> ". "" . " </div>";
        
        // Return HTML
         
        $extraOut =  ( makeJSForSlider($slider_html_id ));
        //esc_attr
