...
<div tabindex="0" class="cycloneslider cycloneslider-template-standard cycloneslider-width-<?php echo esc_attr( $slider_settings['width_management'] ); ?>"
    ...
    <div class="cycloneslider-slides cycle-slideshow"
        ...
        data-cycle-pager-template="<span id='cycloneslider-pager-link-<?php echo esc_attr( $slider_html_id ); ?>-{{slideNum}}'> <a href=# > <script>
        
     
      jQuery(document).ready(function($){
      var slider = $('#<?php echo esc_attr( $slider_html_id ); ?>').children(':first'); 
      jQuery(slider).ready(function($){
       
      var cSlide = $(slider).children().eq(({{slideNum}} -1));
      
      var cSlideCode =  cSlide.find('code').eq(0);
      
      if(cSlideCode.text().match(/\d+/) != null )
      {
        var el = $( '#cycloneslider-pager-link-<?php echo esc_attr( $slider_html_id ); ?>-{{slideNum}}' );
        
        
        el.css('border','1px solid #00ff00');
      ...
      // alert(el);  alert('slide no match at' + ' {{slideNum}} '+ cSlideCode.text());  alert('slide match at'+ ' {{slideNum}} '+ cSlideCode.text().match(/\d+/));
      </script>
       &bull; x </a></span>" 
        ...
        <?php foreach($slides as $slide): ?>
            ...
            <?php elseif ( 'testimonial' == $slide['type'] ) : ?>
               ...
               <!-- </div> -->
            <?php endif; ?>
        <code data-GeoRelevancy="<?php echo $slide['matches_nations']; ?>">
            <?php 
            if($slide['matches_nations']){
               //throw new Exception('matches_nations true.'.$slide['matches_nations']);
            }else{
                //throw new Exception('matches_nations false.'.$slide['matches_nations']);
            }
            //throw new Exception('matches_nations.'.$slide['matches_nations']);
            echo $slide['matches_nations'];
